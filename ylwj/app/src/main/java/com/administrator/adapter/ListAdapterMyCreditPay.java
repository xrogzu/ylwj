 ...
 * 积分消费adapter
 */
public class ListAdapterMyCreditPay extends BaseAdapter {
    ...
    public static class MyHandler extends Handler {
        private WeakReference<ListAdapterMyCreditPay> mMyPay;

        public MyHandler(ListAdapterMyCreditPay myPay) {
            mMyPay = new WeakReference<ListAdapterMyCreditPay>(myPay);
        }

        @Override
        public void handleMessage(Message msg) {
            ListAdapterMyCreditPay myPay = mMyPay.get();
            if (myPay != null) {
...
//                                        Toast.makeText(myPay.context, "取消订单成功", Toast.LENGTH_SHORT).show();
                                        break;
                                    }
                                }
                            }
...
//                            Toast.makeText(myPay.context, jsonObject.get("message").toString(), Toast.LENGTH_SHORT).show();
                        }
...
//                            Toast.makeText(myPay.application, "确认收货成功", Toast.LENGTH_SHORT).show();
                        } else {
                            ToastUtil.showToast(myPay.context, jsonObject.getString("message"));
//                            Toast.makeText(myPay.context, jsonObject.getString("message"), Toast.LENGTH_SHORT).show();
                        }
                    } catch (JSONException e) {
                        e.printStackTrace();
                    }
    ...
    public ListAdapterMyCreditPay(Context context, BaseApplication appContext, int type) {
        this.context = context;
        ...
        if (mType == Constant.WAIT_PAY) {
            ...
            if (bean.getOrderStatus().equals(OrderStatus.getOrderStatusText(OrderStatus.ORDER_SHIP)) || bean.getOrderStatus().equals(OrderStatus.getOrderStatusText(OrderStatus.ORDER_PAY_CONFIRM))) {
                ...
                vh.iv_pay.setOnClickListener(new View.OnClickListener() {
                    @Override
                    public void onClick(View v) {
                        new AlertDialog.Builder(context).setMessage("确认收货成功？").setPositiveButton("确定", new DialogInterface.OnClickListener() {
                            @Override
                            public void onClick(DialogInterface dialog, int which) {
