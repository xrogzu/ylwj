...
import com.administrator.adapter.ListAdapterMyMoneyPay;
...
public class MyMoneyPay_HistoryFragment extends BaseFragment {
    ...
    private ListAdapterMyMoneyPay adapter;
    ...
    public MyMoneyPay_HistoryFragment(Context context, int which) {
        this.context = context;
        this.which = which;
    }

    public static class MyHandler extends Handler {
        private WeakReference<MyMoneyPay_HistoryFragment> mFragment;

        public MyHandler(MyMoneyPay_HistoryFragment fragment) {
            mFragment = new WeakReference<MyMoneyPay_HistoryFragment>(fragment);
        }

        @Override
        public void handleMessage(Message msg) {
            MyMoneyPay_HistoryFragment fragment = mFragment.get();
            if (fragment != null) {
...
//                            Toast.makeText(context, message, Toast.LENGTH_LONG).show();
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        if (which == Constant.WAIT_PAY) {
            VolleyUtils.NetUtils(appContext.getRequestQueue(), Constant.baseUrl + Constant.myMoneyWaitForPay, new String[]{"page", "num"}, new String[]{"1", "20"}, handler, Constant.GOODS_LIST);
        }
        //已付款
        else if (which == Constant.PATED) {
            VolleyUtils.NetUtils(appContext.getRequestQueue(), Constant.baseUrl + Constant.myMoneyPaid, new String[]{"page", "num"}, new String[]{"1", "20"}, handler, Constant.GOODS_LIST);
        }
        //全部
        else if (which == Constant.ALL_PAY) {
            VolleyUtils.NetUtils(appContext.getRequestQueue(), Constant.baseUrl + Constant.myMoneyPayAll, new String[]{"page", "num"}, new String[]{"1", "20"}, handler, Constant.GOODS_LIST);
        }
        initView();
        return mView;
    }
    ...
    private void initView() {
                ...
                if (currPage <= 1) {
                    if (myLists.size() < 20) {
                        ...
                        if (which == Constant.WAIT_PAY) {//历届
                            VolleyUtils.NetUtils(appContext.getRequestQueue(), Constant.baseUrl + Constant.myMoneyWaitForPay, new String[]{"page", "num"}, new String[]{currPage + "", "20"}, handler, Constant.GOODS_LIST);
                        }
                        if (which == Constant.PATED) {
                            VolleyUtils.NetUtils(appContext.getRequestQueue(), Constant.baseUrl + Constant.myMoneyPaid, new String[]{"page", "num"}, new String[]{currPage + "", "20"}, handler, Constant.GOODS_LIST);
                        }
                        if (which == Constant.ALL_PAY) {
                            VolleyUtils.NetUtils(appContext.getRequestQueue(), Constant.baseUrl + Constant.myMoneyPayAll, new String[]{"page", "num"}, new String[]{currPage + "", "20"}, handler, Constant.GOODS_LIST);
                        }
                    }
        ...
        adapter = new ListAdapterMyMoneyPay(getActivity(), appContext, which);
